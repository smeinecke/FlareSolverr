...
class _SessionManager:
    ...
    def create(
        ...
        When running behind a load balancer (e.g., HAProxy), the client
        generates a UUID automatically so the session ID is known from the
        first request. This ensures the load balancer routes all requests
        for this session to the same backend. You can override the ID by
        passing `session_id` explicitly.

        Args:
            session_id: Optional custom session ID. If not provided, a random UUID is generated by the client.
        ...
        payload["session"] = session_id or str(uuid.uuid4())
    ...
    def cdp(self, session_id: str, cdp: dict[str, Any]) -> V1Response:
        ...
        Args:
            ...
            cdp: Dictionary with "cmd" (CDP command name) and optional "params"
                 (dictionary of CDP command parameters).
