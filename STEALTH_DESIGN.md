...
| `performance.now` timing jitter | Native (Blink) | C++ patch `--stealth-performance-now-jitter` bounds the reported value to `raw + 0.5 ms` while preserving monotonicity in `Performance::now()` | A | Ablating it caused `hasInconsistentTimingResolution: true` on deviceandbrowserinfo.com. The previous unbounded random walk inflated 1000 tight-loop calls by ~1.3 s; the new implementation caps the lead at 0.5 ms over Chromium's clamped monotonic clock. |
