...
class PatchApplier:
    ...
    def run_patches(self) -> None:
        ...
        # ──────────────────────────────────────────────────────────────────────────────
        # Patch 14: V8 flag to make Error.prepareStackTrace non-writable and undefined
        # The JavaScript shim installs an observable non-configurable accessor. This
        # native V8 patch instead installs a non-configurable, non-writable data
        # property with value undefined on the global Error constructor during
        # context bootstrap. It applies to all realms and removes the JS-level
        # Function.prototype.toString / descriptor disguise issues.
        # File: v8/src/flags/flag-definitions.h, v8/src/init/bootstrapper.cc
        # Renderer processes receive the flag via --js-flags.
        # ──────────────────────────────────────────────────────────────────────────────
        ...
        self.patch(
            ...
            "  if (v8_flags.stealth_error_prepare_stack_trace) {\n"
            "    DirectHandle<JSObject> Error = isolate->error_function();\n"
            "    DirectHandle<Object> undefined_value = isolate->factory()->undefined_value();\n"
            "    JSObject::AddProperty(\n"
            "        isolate, Error, \"prepareStackTrace\", undefined_value,\n"
            "        static_cast<PropertyAttributes>(READ_ONLY | DONT_ENUM | DONT_DELETE));\n"
            "  }",
            "make Error.prepareStackTrace non-writable undefined when stealth flag is set",
        )
