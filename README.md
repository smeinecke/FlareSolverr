...
| CHALLENGE_PROBE_GRACE | 12                  | Seconds after challenge detection before the resolver starts probing the challenge DOM or attempting verify clicks. Automatic Turnstile verification needs an undisturbed window — probing it too early can stall the challenge. |
...
| STEALTH_OMIT_FLAGS | none                   | Comma-separated list of `--stealth-*` switch names to omit at launch (without dashes), for runtime ablation of switch-gated native patches. Example: `STEALTH_OMIT_FLAGS=stealth-viewport-size,stealth-no-media-devices`. |
