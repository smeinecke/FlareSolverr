...
| AGENT_CHECK_PORT   | none                   | Port for the HAProxy agent-check TCP listener. When set, FlareSolverr starts a raw TCP socket that HAProxy can poll for load-state (`ready`/`50%`/`drain`). The state reflects the **more restrictive** of request load (`MAX_PARALLEL_REQUESTS`) and session saturation (`SESSION_MAX_COUNT`). |
